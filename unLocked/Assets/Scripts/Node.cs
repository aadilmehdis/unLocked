...
public class Node : MonoBehaviour
{
    ...
    public bool isLevelSubGoal = false;

    public bool isStealthNode = false;

    public bool isTeleportationNode = false;

    public bool isPivotPoint = false;

    public bool isInitNode = false;

    public bool isGateOpener = false;

    public Node teleportationDestination;
    ...
    // Camera movement ease type
    public iTween.EaseType cameraEaseType = iTween.EaseType.easeOutQuint;

    public float cameraMovementWait = 1.5f;
    ...
    IEnumerator UpdateCameraRoutine()
    {
        if (isPivotPoint)
        {
            iTween.MoveTo(m_camera.gameObject, iTween.Hash(
                "x", cameraPosition.x,
                "y", cameraPosition.y,
                "z", cameraPosition.z,
                "delay", cameraDelay,
                "easetype", cameraEaseType
            ));
            yield return new WaitForSeconds(cameraMovementWait);
            
            iTween.RotateTo(m_camera.gameObject, iTween.Hash(
                "x", cameraRotation.x,
                "y", cameraRotation.y,
                "z", cameraRotation.z,
                "delay", cameraDelay,
